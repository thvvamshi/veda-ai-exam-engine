...
exports.UnifiedAIProvider = void 0;
...
class UnifiedAIProvider {
        ...
        try {
            ...
            const completion = await groq.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are a professional exam paper generator. Always return valid JSON only.",
                    },
                    {
                        role: "user",
                        content: prompt,
    ...
    normalizeResponse(rawResponse) {
        const jsonString = (0, extractJson_1.extractJson)(rawResponse);
        ...
        parsedResponse.answerKey =
            Array.isArray(parsedResponse.answerKey) &&
                ...
                ? parsedResponse.answerKey
                : parsedResponse.sections.flatMap((section) => section.questions.map((question) => ({
                    question: question.questionText,
                    answer: question.correctAnswer || "Refer explanation",
                })));
        parsedResponse.totalQuestions = parsedResponse.sections.reduce((acc, section) => acc + section.questions.length, 0);
        parsedResponse.totalMarks = parsedResponse.sections.reduce((acc, section) => acc +
            section.questions.reduce((qAcc, question) => qAcc + question.marks, 0), 0);
        return parsedResponse;
    }
}
exports.UnifiedAIProvider = UnifiedAIProvider;
